...
# Acessando o arquivo Excel
planilha_fornecedores = load_workbook('./fornecedores.xlsx')
# Acessando a planilha Indexada
...
for linha in pagina_fornecedores.iter_rows(min_row=2, values_only=True):
    ...
    texto_contrato = f"""
Este contrato de prestação de serviços é feito entre {nome_empresa}, com endereço em {endereço}, {cidade}, {estado}, 
CEP {cep}, doravante denominado FORNECEDOR, e a empresa CONTRATANTE.
...
E-mail: contato@prestadores-sa.com.br

Brásilia, {datetime.now().strftime('%d/%m/%Y')}
"""
